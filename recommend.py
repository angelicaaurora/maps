...
def group_by_centroid(restaurants, centroids):
    ...
    """
    # BEGIN Question 4
    def pairs(restaurant):
        return [find_closest(restaurant_location(restaurant), centroids), restaurant]
    return group_by_first([pairs(restaurant) for restaurant in restaurants])
    # END Question 4
...
def find_centroid(cluster):
    ...
    """
    # BEGIN Question 5
    location = [restaurant_location(restaurant) for restaurant in cluster]
    longitude = [i[0] for i in location]
    latitude = [i[1] for i in location]
...
def k_means(restaurants, k, max_updates=100):
    ...
    while previous_centroids != centroids and n < max_updates:
        ...
        centroids = [find_centroid(cluster) for cluster in group]
        # END Question 6
        n += 1
    return centroids
...
def find_predictor(user, restaurants, feature_fn):
    ...
    b, a, r_squared = S_xy/S_xx, mean(ys) - S_xy/S_xx*mean(xs), S_xy**2/(S_xx * S_yy)  # REPLACE THIS LINE WITH YOUR SOLUTION
    # END Question 7
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    return find_predictor(user, reviewed, max(feature_fns, key = lambda x: find_predictor(user, reviewed, x)[1]))[0]
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    for i in restaurants:
        if i in reviewed:
            dict[restaurant_name(i)] = user_score(user, restaurant_name(i))
        else:
